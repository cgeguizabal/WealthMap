...
public class AddDeductionHandler : ICommandHandler<AddDeductionCommand, JobDto>
{
    private readonly IJobRepository _jobs;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IUserClock _clock;

    public AddDeductionHandler(IJobRepository jobs, IUnitOfWork unitOfWork,
        IUserClock clock)
    {
        _jobs = jobs;
    ...
    public async Task<JobDto> Handle(AddDeductionCommand request, CancellationToken ct)
    {
        var job = await _jobs.GetByIdForUserAsync(request.JobId, request.UserId, ct)
            ?? throw new NotFoundException("Job", request.JobId);

        job.AddDeduction(request.Name, (DeductionType)request.Type, request.Value);
